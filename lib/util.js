...
const cinepakArgs = [
    ...
    '-stats',
    '-vf',
    "pp=ac,pp=ac,pp=ac,hqdn3d=5,eq=contrast=256/220:brightness=1/512:saturation=256/224:gamma=16/16,scale=320:-4:sws_flags=spline+accurate_rnd+full_chroma_int+full_chroma_inp:sws_dither=2:in_range=0:out_range=2,minterpolate=fps=15:mi_mode=mci:me_mode=bidir:me=ntss:vsbmc=1,xbr=2,format=pix_fmts=rgb24",
    '-c:v',
    'cinepak',
    '-q:v',
    '30',
    '-af',
    "pan=mono|FC<1.414FC+FR+FL+0.5BL+0.5SL+0.25LFE+0.125BR,aresample=resampler=swr:osr=22050:cutoff=0.990:dither_method=shibata",
    '-c:a',
    'adpcm_ima_qt',
    '-s',
    '320x240',
...
function openFfmpegConversion(source, destination, format, binary) {
    const ffmpeg = binary || 'ffmpeg';
    let args = ['-i', source, '-y'];
    if (format == 'cinepak') {
        args.push(...cinepakArgs);
...
module.exports.openFfmpegConversion = openFfmpegConversion;
